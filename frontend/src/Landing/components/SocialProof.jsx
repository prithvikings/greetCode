...
const CompanyLogos = () => {
  // Note: Using high-quality SVG/PNGs is key. 
  // I've ensured these are clean, widely used URLs for testing.
  const companies = [
  ...
  return (
    <div className="relative w-full overflow-hidden mb-24 border-y border-zinc-200 bg-white py-8">
      {/* Fade Masks */}
      <div className="absolute inset-y-0 left-0 w-36 bg-gradient-to-r from-white to-transparent z-10" />
      <div className="absolute inset-y-0 right-0 w-36 bg-gradient-to-l from-white to-transparent z-10" />
      ...
      <div className="flex w-full">
        ...
        >
          {companies.map((company, i) => (
            <div key={i} className="relative w-32 h-12 flex items-center justify-center grayscale opacity-40 hover:grayscale-0 hover:opacity-100 transition duration-300 ease-in cursor-pointer">
               <img 
                 src={company.url} 
                 alt={company.name} 
...
const ReviewCard = ({ name, role, company, image, quote, stats, index }) => (
  <motion.div
    ...
    className="bg-white p-6 rounded-2xl border border-zinc-100 shadow-xl shadow-zinc-200/40 relative group"
  >
    ...
    <div className="flex items-center gap-4 mb-4">
      ...
      <div>
        ...
        <p className="text-xs text-zinc-500 font-medium">{role} @ {company}</p>
      </div>
    </div>

    {/* Content */}
    <p className="text-zinc-600 text-sm leading-relaxed mb-6 relative z-10 font-inter">
      "{quote}"
    </p>
    ...
    <div className="pt-4 border-t border-zinc-50 flex items-center justify-between">
       ...
       <div className="text-xs font-mono text-sky-600 bg-sky-50 px-2 py-1 rounded border border-sky-100 flex items-center gap-1">
  ...
  return (
    <section className="py-24 bg-zinc-50 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center px-6 mb-12">
        <h2 className="text-4xl md:text-5xl font-poppins font-medium text-zinc-900 tracking-tight mb-4">
          Trusted by engineers at top companies.
        </h2>
        <p className="text-zinc-500 font-poppins text-sm w-md mx-auto">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* We map the reviews into columns */}
      ...
      <div className="mt-20 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-zinc-200 shadow-sm cursor-pointer">
             ...
             <span className="text-sm text-zinc-600 font-medium pl-2">
                 Rated <span className="text-zinc-900 font-bold">4.9/5</span> by active learners
             </span>
          </div>
      </div>
